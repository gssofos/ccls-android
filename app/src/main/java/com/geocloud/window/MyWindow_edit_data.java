...
public class MyWindow_edit_data {
    ...
    public MyWindow_edit_data(MyParams params) {
        ...
        this.layout_params = new RelativeLayout.LayoutParams(this.windowWidth, params.imagePixelHeight - 350);
        ...
        tmp = new RelativeLayout.LayoutParams(this.windowWidth, this.lineHeight);
        tmp.leftMargin = 20;
        tmp.topMargin = 20;
        ...
        tmp = new RelativeLayout.LayoutParams(this.windowWidth - 30, params.imagePixelHeight - 350 - 300);
        tmp.leftMargin = 20;
        tmp.topMargin = this.lineHeight;
        this.layout.addView(sv, tmp);


        this.layout_sv = new RelativeLayout(params.app.getBaseContext());	/*this.layout.setOrientation(1);*/
        tmp = new RelativeLayout.LayoutParams(this.windowWidth, params.imagePixelHeight - 350 - 170);
        tmp.leftMargin = 0;
        tmp.topMargin = 0;
        ...
        redrawTax.setText("Taximet");
        redrawTax.setTextColor(Color.argb(255, 255, 255, 255));
        redrawTax.setTextSize(this.textSize);
        tmp = new RelativeLayout.LayoutParams(300, 130);
        tmp.leftMargin = 25;
        tmp.topMargin = params.imagePixelHeight - 500;
        ...
        recalcOdefsi.setTextSize(this.textSize);
        tmp = new RelativeLayout.LayoutParams(300, 130);
        tmp.leftMargin = 350;
        tmp.topMargin = params.imagePixelHeight - 500;
    ...
    public void set_data(int index) {
        ...
        if (index >= 0) {
            ...
            for (i = params.msets.size() - 1; i >= 0; i--) {
                ...
                if (ms.stasi == params.selectedStasi) {
                    ...
                    for (k = 0; k <= ms.itemStaseis.size() - 1; k++) {
                        ...
                        tmp = params.util.addTextView(layout_sv,
                                40, (counterPer) * 90 + counter * 70 + 10,
                                400, 50,
                        ...
                        tmp.setOnClickListener(new OnClickListener() {
                                                   ...
                                                   public void onClick(View v) {
                                                       // TODO Auto-generated method stub
                                                       int gid = v.getId();
                                                       int p = (int) Math.floor(gid / 10000);
                                                       int m = gid - p * 10000;
                                                       //Log.i("p:" + p,"m:" + m);
                        ...
                        tmp.setOnClickListener(new OnClickListener() {
                                                   ...
                                                   public void onClick(View v) {
                                                       ...
                                                       MyMeasurement tmp = get_params().msets.get(p).itemStaseis.get(m);
                                                       if (tmp.odefsi_use) {
                                                           tmp.db_set_odefsi_use(false);
                                                           tmp.odefsi_use = false;
                                                       } else {
                                                           tmp.odefsi_use = true;
                                                           tmp.db_set_odefsi_use(true);
                                                       }
                                                       get_params().window_edit_data.set_data(get_params().selectedStasi);
                                                       get_params().renderer.points.clear();
                                                       get_params().odefsi.redrawfromstasi(false, get_params().staseis.get_stasi_by_id(get_params().msets.get(p).stasi_id));
            ...
            for (i = 0; i <= params.activeProject.st.size() - 1; i++) {
                ...
                cb = new CheckBox(params.app.getBaseContext());    //text1.setText("");		//text1.setBackgroundColor(Color.argb(100,0, 0, 0));
                lay = new RelativeLayout.LayoutParams(130, 130);
                lay.leftMargin = 10;
                lay.topMargin = (counter) * 140 + 110 - 3;
                layout_sv.addView(cb, lay);
